...
InventoryPanel::InventoryPanel(InventoryComponent* inventoryToDisplay)//,  int const topLeftX, int const topLeftY
{
	...
	Room* inv = new Room("Inventory", char(239), char(248), char(248), char(248), char(248), char(248), char(248), 0, 0, app.GetConsoleWidth() - 30, 0,8,3);
	inv->CreateFromVector2D(app.ReadFileToVector2D("Scenes/playerinventory.txt"));
